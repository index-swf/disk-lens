...
use crate::models::{ScanProgress, ScannerError, TreeNode};
use std::path::Path;
...
///
/// Returns the aggregated tree together with the name of the strategy that
/// actually produced it (`"usn"` or `"parallel"`), so the frontend / caller can
...
pub fn scan_drive(
    ...
    precise: bool,
    threads: Option<u32>,
) -> Result<(TreeNode, String), ScannerError> {
    ...
    match method {
        "usn" => {
            let tree = usn::scan_usn(&path, window)?;
            Ok((tree, "usn".to_string()))
        }
        "parallel" => {
            let tree = parallel::scan_parallel(&path, window, precise, threads)?;
            Ok((tree, "parallel".to_string()))
        }
        _ => {
            // "auto"
            if usn::is_ntfs(&path) {
                if let Ok(tree) = usn::scan_usn(&path, window.clone()) {
                    return Ok((tree, "usn".to_string()));
                }
